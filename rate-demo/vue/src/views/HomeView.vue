<template>
  <div style="width: 1000px;margin: 0 auto">
    <div style="width: 500px;margin: 10px auto;border: 1px solid #ccc;padding: 20px">
      ...
      <div style="margin: 10px 0">商品评分：
        <el-rate style="display: inline-block" :colors="comment.colors" v-model="value" disabled show-text></el-rate>
      </div>
    </div>

    <div style="margin: 20px 0">
      <el-rate v-model="comment.rate" :colors="comment.colors" allow-half show-text
               :texts="['极差', '失望', '一般', '很好', '惊喜']"></el-rate>
            ...
            <div v-if="item.children.length"
              ...
              <div v-for="sub in item.children" :key="sub.id">
                <div style="padding: 5px 0">
                  <b style="cursor: pointer" @click="reply(sub.pid,sub.target)">{{ sub.username }}</b>
                  <span>回复
